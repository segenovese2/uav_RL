...
class UAVEnv(gym.Env):
    ...
    def __init__(
        ...
        self.uav_height = uav_height  # fixed flight altitude -- sets minimum distance floor
    ...
    # ---------------- PAPER-ACCURATE REWARD ----------------
    def _calculate_reward(self):
        pos = self.current_pos
        sum_rate = 0.0

        for user in self.users:
            # 3D distance: horizontal grid distance + fixed UAV altitude
            # this prevents d_k collapsing to ~0 when UAV is directly above a user,
            # which would otherwise produce unbounded reward and cause corner camping
    ...
    def _init_pygame(self):
        ...
        pygame.display.set_caption("UAV Trajectory - Paper Accurate Version")
    ...
    def render(self):
        ...
        for x in range(self.grid_size + 1):
            pygame.draw.line(self.screen, (200,200,200), (x*self.cell_size,0), (x*self.cell_size,self.window_size))
        for y in range(self.grid_size + 1):
            pygame.draw.line(self.screen, (200,200,200), (0,y*self.cell_size), (self.window_size,y*self.cell_size))
        ...
        if len(self.trajectory) > 1:
            ...
            for pos in self.trajectory:
                px = pos[0]*self.cell_size + self.cell_size//2
                py = (self.grid_size-1-pos[1])*self.cell_size + self.cell_size//2
                pts.append((px,py))
        ...
        pygame.draw.circle(self.screen, (255,165,0), (ux,uy), 8)

        # text info
        info = self.font.render(f"Step {self.steps}/{self.max_steps}", True, (0,0,0))
